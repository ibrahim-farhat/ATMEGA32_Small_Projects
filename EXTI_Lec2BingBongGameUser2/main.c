...
void main(void)
{
	...
	/*Initial condition to the second microcontrolloer state*/
	DIO_u8SetPinValue(DIO_u8PORTC, DIO_u8PIN0, DIO_u8PIN_LOW);
	Main_u8StateFlag = 0;
	CLCD_u8GoToXY(15, 0);
	CLCD_voidSendSpecialCharacter(Local_u8RacketCharacter, 1, 15, 0);
	...
	while(1)
	{
		if(Main_u8StateFlag == 1)
		{
			 ...
			 * */
			for(Local_u8LaunchCounter = 15; Local_u8LaunchCounter >= 1; Local_u8LaunchCounter--)
			{
				CLCD_u8GoToXY((Local_u8LaunchCounter - 1), Local_u8CatchAreaFlag);
				CLCD_voidSendSpecialCharacter(Local_u8BallCharacter, 2, (Local_u8LaunchCounter - 1), Local_u8CatchAreaFlag);
				_delay_ms(100);
				CLCD_voidSendSpecialCharacter(Local_u8EraseCharacter, 0, (Local_u8LaunchCounter - 1), Local_u8CatchAreaFlag);
			}
...
void Main_voidStateISR(void)
{
	...
	if(Main_u8StateFlag != 1)
	{
		for(Local_u8CatchCounter = 0; Local_u8CatchCounter < 15; Local_u8CatchCounter++)
		{
...
void Main_voidCatchButtonISR(void)
{
	if(Main_u8CatchAreaFlag == 0)
	{
		...
		CLCD_u8GoToXY(15, 0);
		CLCD_voidSendSpecialCharacter(Local_u8EraseCharacter, 0, 15, 0);

		CLCD_u8GoToXY(15, 1);
		CLCD_voidSendSpecialCharacter(Local_u8RacketCharacter, 1, 15, 1);
	}
	else if(Main_u8CatchAreaFlag == 1)
	{
		...
		CLCD_u8GoToXY(15, 1);
		CLCD_voidSendSpecialCharacter(Local_u8EraseCharacter, 0, 15, 1);
